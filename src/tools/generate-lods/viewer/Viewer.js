...
/**
 * A self-contained Three.js preview: one per viewport slot. The "Original"
 * and "LOD" viewers are each configured identically (camera, lighting,
 * neutral studio background) so switching between LOD levels — or comparing
 * either against the source — is a like-for-like visual comparison.
 */
export class Viewer {
  ...
  setModel(object3d) {
    ...
    // Re-apply the current display mode to the new model.
  ...
  // ---------------------------------------------------------------------
  // Display controls. Additive — the tools that only need a plain preview
  // never call these and are unaffected.
  // ---------------------------------------------------------------------
  ...
  setWireframeMode(mode) {
    ...
    // Wireframe is drawn as a separate overlay rather than by flipping
    // material.wireframe, so "solid + wireframe" can show both at once and
    // the line colour stays independent of the model's own materials.
  ...
  /** Re-frames the current model. */
  resetCamera() {
    if (this.currentModel) this.autoFrame(this.currentModel);
  }

  /** Copies this viewer's camera pose onto another viewer. */
  copyCameraTo(other) {
