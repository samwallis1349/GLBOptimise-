...
const toolButton = (label, iconName, attrs) => `
  <button class="aa-tool" type="button" ${attrs}>${icon(iconName)}<span>${label}</span></button>`;

const iconAction = (action, iconName, label) => `
  <button class="aa-icon-btn" type="button" data-action="${action}" title="${label}" aria-label="${label}" disabled>${icon(iconName)}</button>`;

/**
 * One app window: header, library rail, viewport, inspector and a bottom
 * dock share a single frame and are split by hairlines rather than floating
 * as separate cards. Arrange controls live in the viewport and only appear
 * once two or more objects are selected.
 */
  ...
  <div class="aa-app" id="aa-app">
    <header class="aa-topbar">
      <div class="aa-brand">
        ...
        <div><strong>Asset <em>Arranger</em></strong><small>GLB Builder &amp; Merger</small></div>
      </div>
      <nav class="aa-steps" aria-label="Arranger steps">
        <button class="aa-step" type="button" data-tab="import"><b>01</b><span>Import</span></button>
        <i aria-hidden="true"></i>
        <button class="aa-step is-active" type="button" data-tab="arrange"><b>02</b><span>Arrange</span></button>
        <i aria-hidden="true"></i>
        <button class="aa-step" type="button" data-tab="export"><b>03</b><span>Export</span></button>
      </nav>
    ...
    <aside class="aa-rail aa-library" id="aa-library">
      ...
      <button id="aa-drop" class="aa-drop" type="button">
        ${icon('cloud')}
        <strong>Drop GLB / glTF / FBX</strong>
        <small>or click to add · up to ${ARRANGER_CONFIG.maxAssets}</small>
      </button>
      <div class="aa-lib-tools">
      ...
      <div id="aa-assets-empty" class="aa-empty">
        <div class="aa-empty-art" aria-hidden="true"><span></span><span></span><span></span></div>
        <p>Your library fills up here. Nothing enters the scene until you drag it in or press <b>+</b>.</p>
      </div>
    </aside>

    <section class="aa-viewport" id="aa-viewport">
      <div id="aa-canvas" class="aa-canvas"></div>
      <div class="aa-vignette" aria-hidden="true"></div>
      <div class="aa-view-bar">
        <div class="aa-views aa-glass">
        ...
        <div class="aa-view-actions aa-glass">
          <button id="aa-frame-selected" class="aa-icon-btn aa-icon-btn--ghost" type="button" title="Frame selected (F)" aria-label="Frame selected">${icon('target')}</button>
          <button id="aa-frame-all" class="aa-icon-btn aa-icon-btn--ghost" type="button" title="Frame all" aria-label="Frame all">${icon('frame')}</button>
          <button id="aa-grid-toggle" class="aa-icon-btn aa-icon-btn--ghost is-on" type="button" title="Toggle grid" aria-label="Toggle grid">${icon('grid')}</button>
        </div>
      </div>
      <div id="aa-drop-overlay" class="aa-drop-overlay" hidden><span id="aa-drop-label">Drop to place</span></div>
      <div id="aa-hint" class="aa-hint">${icon('mouse')}<span>Drag an asset in from the library<br>or press + on its card</span></div>
      <div id="aa-busy" class="aa-busy" hidden><span class="aa-spinner"></span><span id="aa-busy-label">Loading…</span></div>

      <div class="aa-dock-float">
        <div class="aa-arrange aa-glass" id="aa-arrange" aria-label="Arrange selected">
          <span class="aa-arrange-label">${icon('distribute')}<span id="aa-arrange-meta">Arrange</span></span>
          <label class="aa-gap" title="Visible gap between objects"><span>Gap</span><input id="aa-gap" type="number" min="0" step="0.05" value="${ARRANGER_CONFIG.defaultGap.toFixed(2)}"><em>m</em></label>
          ...
          ${toolButton('Align X', 'alignX', 'data-action="align-x" title="Line selected up on the primary object\'s X"')}
          ${toolButton('Align Z', 'alignZ', 'data-action="align-z" title="Line selected up on the primary object\'s Z"')}
          ${toolButton('Distribute', 'distribute', 'data-action="distribute" title="Equal visible gaps between the outermost objects"')}
          <button class="aa-btn aa-btn--primary aa-btn--sm" type="button" data-action="arrange">Arrange</button>
        </div>
        <div class="aa-toolbar aa-glass" role="toolbar" aria-label="Viewport tools">
          ${toolButton('Select', 'pointer', 'data-mode="select" title="Select (Q)"')}
          ${toolButton('Move', 'move', 'data-mode="move" title="Move (W)"')}
          ${toolButton('Rotate', 'rotate', 'data-mode="rotate" title="Rotate (E)"')}
          ${toolButton('Scale', 'scale', 'data-mode="scale" title="Scale (R)"')}
          <span class="aa-sep"></span>
          ${toolButton('Duplicate', 'copy', 'data-action="duplicate" title="Duplicate (Ctrl+D)"')}
          ${toolButton('Dup + Gap', 'copyGap', 'data-action="duplicate-gap" title="Duplicate and repeat the last offset"')}
          ${toolButton('Delete', 'trash', 'data-action="delete" title="Delete (Del)"')}
        </div>
      </div>
    </section>

    <aside class="aa-rail aa-side">
      <section class="aa-section aa-outliner">
        <div class="aa-section-head"><h2>Scene <span id="aa-instance-count" class="aa-count">(0)</span></h2></div>
      ...
      <section class="aa-section aa-transform" id="aa-transform">
        <div class="aa-section-head"><h2>Transform</h2><span id="aa-transform-target" class="aa-count">Nothing selected</span></div>
        <div class="aa-field-label">Position <em>m</em></div>
        ...
        <div class="aa-field-label">Rotation <em>°</em></div>
        ...
        <div class="aa-action-row" role="group" aria-label="Object actions">
          ${iconAction('duplicate', 'copy', 'Duplicate (Ctrl+D)')}
          ${iconAction('duplicate-gap', 'copyGap', 'Duplicate + Gap')}
          ${iconAction('reset', 'reset', 'Reset rotation and scale')}
          ${iconAction('centre', 'centre', 'Centre on plane')}
          ${iconAction('delete', 'trash', 'Delete (Del)')}
        </div>
      </section>

      <section class="aa-section aa-anim" id="aa-anim">
        ...
        <div id="aa-anim-body" class="aa-anim-body" hidden>
          ...
          <p class="aa-note">Drop a GLB / FBX animation on the viewport with this model selected to add a compatible clip.</p>
        </div>
      </section>
    </aside>

    <footer class="aa-dock">
      <section class="aa-dock-group aa-ground">
        <div class="aa-plane-card" aria-hidden="true"><div class="aa-plane" id="aa-plane-preview"></div></div>
        <div class="aa-dock-fields">
          <span class="aa-dock-title">Ground</span>
          <div class="aa-inline">
            <select id="aa-ground-texture" class="aa-select" aria-label="Ground texture">${GROUND_TEXTURES.map((t) => `<option value="${t.id}"${t.id === 'custom' ? ' disabled' : ''}${t.id === 'dust' ? ' selected' : ''}>${t.label}</option>`).join('')}</select>
            <button id="aa-upload-texture" class="aa-icon-btn" type="button" title="Upload your own texture" aria-label="Upload texture">${icon('cloud')}</button>
            <input id="aa-texture-file" type="file" accept="${ARRANGER_CONFIG.textureAccept}" hidden>
            <div class="aa-stepper" title="Plane size">
          ...
          <div class="aa-toggles">
            <label><input id="aa-grid-visible" type="checkbox" class="aa-switch" checked><span>Grid</span></label>
            <label><input id="aa-ground-visible" type="checkbox" class="aa-switch" checked><span>Ground</span></label>
          </div>
        </div>
      </section>

      <section class="aa-dock-group aa-export" id="aa-export">
        <div class="aa-dock-fields">
          <span class="aa-dock-title">Export</span>
        ...
        <div class="aa-export-actions">
          ...
          <button id="aa-export-scene" class="aa-btn aa-btn--primary aa-btn--export aa-btn--hero" type="button" disabled>${icon('download')}<span><strong>Export Scene</strong><small id="aa-scene-meta">All visible objects combined</small></span></button>
        </div>
      </section>
    </footer>
  </div>
